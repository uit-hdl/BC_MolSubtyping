...
class MultiModel(BaseModel):
	...
	# INCEPTION_V3
        ...
        #    model.Conv2d_1a_3x3 = nn.Conv2d(3, 32, kernel_size=3, stride=2, padding=0)
        #    named_children = list(model.named_children())
        #    first_part = nn.Sequential(OrderedDict(named_children[:-2]))
        #    second_part = nn.Sequential(
        #        OrderedDict([named_children[-2], ('flatten', nn.Flatten()), named_children[-1]]))
    ...
    def get_last_activ_layers(self):
        layers = []
        for model in self.models:  # Iterate over the models in the ModuleList
            # Assuming each model has an 'activations' attribute
            if hasattr(model, 'activations') and hasattr(model.activations, 'layer4'):
                if len(model.activations.layer4) > 1:
                    layers.append(model.activations.layer4[1])  # Append the desired layer
                else:
                    print("*****************************Layer4 does not have a second element in one of the models.")
            else:
                print("+++++++++++++++++++++++++++++++++Layer 'layer4[1]' does not exist in one of the models.")
        return layers
